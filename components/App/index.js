...
export default class extends React.Component {
  constructor(props) {
    ...
    const TIME_NOW = DateTime.local().setZone('America/Los_Angeles')
    const { activeBlock, activeBlockId } = this.initializeStateFromStore(props.store.dates, TIME_NOW)
  ...
  initializeStateFromStore( dates, TIME_NOW ) {
    let activeBlockId = null

    // filter store down to one `Date{ blocks[ [] ], venues[ {} ] }`
    const activeBlock = dates.filter(({ blocks }) => blocks.some(([ start, end ], index) => {
      // determines if a performance is occurring
      if (TIME_NOW >= DateTime.fromISO(start) && TIME_NOW < DateTime.fromISO(end)) {
        activeBlockId = index
        return true
      }
    }))[0]
  ...
  componentDidMount() {
    setInterval(() => {
      ...
      const newTime = DateTime.local().setZone('America/Los_Angeles').plus({ days: 1 });
